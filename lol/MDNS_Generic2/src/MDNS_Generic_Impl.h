...
int MDNS::begin(const IPAddress& ip, const char* name)
{
  ...
  if (statusCode)
  {
    ...
    // KH debug
    MDNS_LOGWARN1("::begin: UDP beginMulticast statusCode=", (statusCode == 1) ? "OK" : "Error");    
  }
...
MDNSError_t MDNS::_sendMDNSMessage(uint32_t /*peerAddress*/, uint32_t xid, int type, int serviceRecord)
{
  ...
  // KH debug
  if(should_debug){
    MDNS_LOGINFO1("::_sendMDNSMessage: xid=", dnsHeader->xid);
    MDNS_LOGINFO1("::_sendMDNSMessage: queryCount=", dnsHeader->queryCount);
    MDNS_LOGINFO1("::_sendMDNSMessage: answerCount=", dnsHeader->answerCount);
    MDNS_LOGINFO1("::_sendMDNSMessage: authorityCount=", dnsHeader->authorityCount);
    MDNS_LOGINFO1("::_sendMDNSMessage: additionalCount=", dnsHeader->additionalCount);
  }
  //////
...
errorReturn:
  ...
  // KH debug
  MDNS_LOGWARN1("::_sendMDNSMessage: statusCode =", statusCode);
  MDNS_LOGWARN0("\r\n");
...
MDNSError_t MDNS::_processMDNSQuery()
{
  ...
  // This gets the latest packet from the Ethernet Library buffer
  // via polling method
  ...
  if (0 == udp_len) 
  {
    //MDNS_LOGINFO0("::_processMDNSQuery: MDNSTryLater\n\r");
  ...
  // Oh good we have a packet
  if (udp_len != 0)
  {
    MDNS_LOGINFO1("::_processMDNSQuery: UDP parsePacket len=", udp_len);
  }
  
  // Allocate memory to hold _remaining UDP packet
  ...
  // read _remaining UDP packet from W5100/W5200 into memory
  this->_udp->read((uint8_t*) udpBuffer, udp_len);
      ...
      MDNS_SERVER_PORT == this->_udp->remotePort())
  {
    ...
    for (i = 0; i < qCnt; i++) 
    {
      ...
      for (j = 0; j < NumMDNSServiceRecords + 2; j++) 
      {
        // check whether this is an A record query (for our own name)
...
errorReturn:
  ...
  for (j = 0; j < NumMDNSServiceRecords + 2; j++) 
  {
    ...
    {
      // Reply with my IPV4 ADDR packet MDNSPacketTypeMyIPAnswer
      if (0 == j)
      ...
      // Reply with MDNSPacketTypeServiceRecord
...
void MDNS::run()
{
  ...
  for (i = 0; i < 2; i++) 
  {
    if (nullptr != this->_resolveNames[i]) 
    {
  ...
  // should we re-announce our services again?
