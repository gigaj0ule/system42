...
static constexpr const uint32_t half_pwm = TIM_1_8_PERIOD_CLOCKS / 2;

// This value is updated by the DC-bus reading ADC.
// Arbitrary non-zero inital value to avoid division by zero if ADC reading is late
float vbus_voltage = 0.01f;

// If this is set true, we should skip PWM cycles as there has been 
// a short circuit fault
bool skip_pwm_cycles = false;
bool cycles_were_skipped = false;
...
// @brief This function samples currents from ADC2 and ADC3
// @ingroup low_level_fast
static inline void sample_voltages(Motor * axis_motor) {
    ...
    // Convert motor currents to ADC values
    output_voltage_ = axis_motor->output_voltage_from_adcval(ADCValue_phB);
    //input_voltage_ = ADCValue_phC;
}
...
void pwm_state_machine(bool injected, uint16_t now) {
    ...
    if(injected) {
        ...
        switch(pwm_adc_state_tracker_) { 
            ...
            case 0:
                ...
                current_measurement_stack_index_ = 5;
            ...
            case 8:
                ...
                //sample_current_ = true;
                current_measurement_stack_index_ = 3;  
            ...
            case 10:
                ...
                //sample_current_ = true;
                current_measurement_stack_index_ = 4;             
                // Debugging
    ...
    if(do_dc_calibration_) {     
        ...
        if(injected) {
            ...
            axis_motor->V_out_DC_calib_ += (output_voltage_ - axis_motor->V_out_DC_calib_) * calib_filter_k;
        }
        //#ifdef USE_SINGLE_AXIS 
        //}
        //#endif
    ...
    if(injected) {
        ...
        // Sample Voltages
        axis_motor->buffered_output_voltage_meas_[current_measurement_stack_index_] = output_voltage_ - axis_motor->V_out_DC_calib_;
    }
    ...
    if(axis_num == 0){
        ...
        if(update_run_control_loop_) {
            ...
            float current_limit = axis_motor->config_.cbc_current_lim;
            ...
            for(int i = 0; i < 6; i++) {
                ...
                // The 2.0f here is because we double-sample currents for lower noise
                ...
                axis_motor->output_voltages_[i] = axis_motor->buffered_output_voltage_meas_[i];
                axis_motor->buffered_output_voltage_meas_[i] = 0.0f;

                // Check now, if the currents are out of bounds we need to 
                // skip some PWM cycles...
                if( axis_motor->buffered_current_meas_[i].phB > current_limit
                    || axis_motor->buffered_current_meas_[i].phC > current_limit
                ) {
                    skip_pwm_cycles = true;
                } 
                else if(cycles_were_skipped) {
                    skip_pwm_cycles = false;
                }
            }
        }

        // Fast current protection: if the currents are out of bounds we need to 
        // skip some PWM cycles...
        if(skip_pwm_cycles) {
    ...
    */
    ...
    if ((brake_duty >= 0.0f) && (brake_duty <= 0.9f)) {
        ...
        // Calculate duty cycle of the brake resistor
            ...
            int high_on = low_off - TIM_APB1_DEADTIME_CLOCKS;
        #else
...
inline __attribute__((__always_inline__)) void vbus_voltage_sense_calculation() {
    ...
    vbus_voltage = 24.0f;//adc_vbus_voltage_scale * (float) ADC1->JDR1;
}