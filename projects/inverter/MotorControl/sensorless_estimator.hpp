...
public:
    ...
    auto make_protocol_definitions() {
        return make_protocol_member_list(
            ...
            //make_protocol_number("phase", &phase_, "rad"),
            //make_protocol_number("pll_pos", &pll_pos_, "rad"),
            //make_protocol_number("vel_estimate_filtered", &vel_estimate_filtered_, "rad/s"),
            // make_protocol_number("pll_kp", &pll_kp_),
            // make_protocol_number("pll_ki", &pll_ki_),

            /*
            make_protocol_object("hfi_debug",
                make_protocol_number("debug_0", &debug_[0]),
                make_protocol_number("debug_1", &debug_[1]),
                make_protocol_number("debug_2", &debug_[2]),
                make_protocol_number("debug_3", &debug_[3]),
                make_protocol_number("debug_4", &debug_[4]),
                make_protocol_number("debug_5", &debug_[5]),
                make_protocol_number("debug_6", &debug_[6]),
                make_protocol_number("debug_7", &debug_[7]),
                make_protocol_number("debug_8", &debug_[8]),
                make_protocol_number("debug_8", &debug_[9])    
            ),
            */
            /*
            make_protocol_object("config",
                make_protocol_number("observer_gain", &config_.observer_gain, "rad/s"),
                make_protocol_number("pll_bandwidth", &config_.pll_bandwidth, "rad/s"),
                make_protocol_number("pm_flux_linkage", &config_.pm_flux_linkage),

                //make_protocol_object("hfi",
                    make_protocol_number("hfi_enabled", &config_.hfi_enabled),
                    make_protocol_number("hfi_calibrated", &config_.hfi_calibrated),
                    make_protocol_number("hfi_signal_snr_target", &config_.test_signal_desired_snr),
                    make_protocol_number("hfi_signal_voltage", &config_.test_signal_voltage, "V"),
                    make_protocol_number("hfi_quiescent_current", &config_.hfi_quiescent_current, "A"),
                    make_protocol_number("hfi_max_calibration_current", &config_.hfi_max_calibration_current, "A"),
                    make_protocol_number("hfi_max_velocity", &config_.hfi_max_velocity),
                    make_protocol_number("hfi_velocity_hysteresis", &config_.hfi_vel_hysteresis),
                    make_protocol_number("hfi_torque_gain", &config_.hfi_torque_gain),
                    make_protocol_number("hfi_torque_decay", &config_.hfi_torque_decay)
                //)
            )*/
        );
    }
