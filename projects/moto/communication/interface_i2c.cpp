...
void i2c_handle_packet(I2C_HandleTypeDef *hi2c) {
    uint32_t received = sizeof(i2c_rx_buffer) - hi2c->XferCount;
    if (received > I2C_RX_BUFFER_PREAMBLE_SIZE) {
        ...
        uint32_t expected_bytes = (TX_BUF_SIZE - 2) < I2C_TX_BUFFER_SIZE ? (TX_BUF_SIZE - 2) : I2C_TX_BUFFER_SIZE;
        write_le<uint16_t>(expected_bytes, i2c_rx_buffer + 4); // hallucinate maximum number of expected response bytes

        i2c1_channel.sink_the_bytestream(i2c_rx_buffer, received);
