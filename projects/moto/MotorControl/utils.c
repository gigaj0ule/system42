...
// STM32F4 FPU functions for warp drive
static inline __attribute__((always_inline)) float _VSQRTF(float op1) {
    float result;
...
static inline __attribute__((always_inline)) float _VABSF(float op1) {
    float result;
    __asm__ volatile ("vabs.f32 %0, %1"  : "=w" (result) : "w" (op1) );
    return(result);
}

inline __attribute__((always_inline)) float fast_absf( float x ) {
    return _VABSF(x);
}
...
inline __attribute__((always_inline)) float fast_atan2(float y, float x) {
    // a := min (|x|, |y|) / max (|x|, |y|)
    float const abs_y = _VABSF(y);
    float const abs_x = _VABSF(x);
    // inject FLT_MIN in denominator to avoid division by zero
    float const a = MACRO_MIN(abs_x, abs_y) / (MACRO_MAX(abs_x, abs_y) + FLT_MIN);
    // s := a * a
    float const s = a * a;
...
inline __attribute__((always_inline)) float fast_asinf(float x) {
    return fast_atan2 (x, _VSQRTF((1.0f + x) * (1.0f - x)));
}

inline __attribute__((always_inline)) float fast_acosf(float x) {
    return fast_atan2 (_VSQRTF((1.0f + x) * (1.0f - x)), x);
}
