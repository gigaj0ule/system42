...
void SM72442_setConfig_duty_cycle_open(uint16_t duty_cycle)
{
	uint64_t bitMaskValue = ~((uint64_t)SM_BITMASK_DUTY_CYCLE_OPEN << SM_BITSHIFT_DUTY_CYCLE_OPEN);
	uint64_t regValue = SM72442_readRegister64(SM_REGISTER_REG_3);
	uint64_t blankValue = regValue & bitMaskValue;
	uint64_t newValue = blankValue | (((uint64_t) duty_cycle & SM_BITMASK_DUTY_CYCLE_OPEN) << SM_BITSHIFT_DUTY_CYCLE_OPEN);
	SM72442_writeRegister64(SM_REGISTER_REG_3, newValue);
}
...
void SM72442_setConfig_open_loop_enable(bool open_loop_enable)
{
	uint64_t bitMaskValue = ~((uint64_t)SM_BITMASK_OPEN_LOOP << SM_BITSHIFT_OPEN_LOOP);
	uint64_t regValue = SM72442_readRegister64(SM_REGISTER_REG_3);
	uint64_t blankValue = regValue & bitMaskValue;
	uint64_t newValue = blankValue | (((uint64_t) open_loop_enable & SM_BITMASK_OPEN_LOOP) << SM_BITSHIFT_OPEN_LOOP);
	SM72442_writeRegister64(SM_REGISTER_REG_3, newValue);
}
...
void SM72442_calibrateCurrentOffsets(void delayFn(uint32_t)){

	// Set open loop duty to 0
	SM72442_setConfig_duty_cycle_open(0);

	// Set open loop to on
	SM72442_setConfig_open_loop_enable(1);

	// Cycle reset to enable open loop
	SM72442_setConfig_override_soft_reset(1);
	SM72442_setConfig_override_soft_reset(0);

	// Wait for reboot
	delayFn(3);

	// Get values
	SM72442_readIV(&SM72442_tare_Vout, &SM72442_tare_Iout, &SM72442_tare_Vin, &SM72442_tare_Iin);
	
	// Turn off open loop mode
	SM72442_setConfig_open_loop_enable(0);

	// Cycle reset to turn off open loop?
	SM72442_setConfig_override_soft_reset(1);
	SM72442_setConfig_override_soft_reset(0);
	...
	// Write new offsets
	// SM72442_setConfig_Vout_offset(SM72442_tare_Vout);
	SM72442_setConfig_Iout_offset(SM72442_tare_Iout);
	// SM72442_setConfig_Vin_offset(SM72442_tare_Vin);
	SM72442_setConfig_Iin_offset(SM72442_tare_Iin);
}