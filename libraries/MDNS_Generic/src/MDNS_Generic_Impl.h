...
MDNSError_t MDNS::_sendMDNSMessage(uint32_t /*peerAddress*/, uint32_t xid, int type, int serviceRecord)
{
  ...
  // KH debug
  if(should_debug){
    MDNS_LOGWARN1("::_sendMDNSMessage: xid=", dnsHeader->xid);
    MDNS_LOGWARN1("::_sendMDNSMessage: queryCount=", dnsHeader->queryCount);
    MDNS_LOGWARN1("::_sendMDNSMessage: answerCount=", dnsHeader->answerCount);
    MDNS_LOGWARN1("::_sendMDNSMessage: authorityCount=", dnsHeader->authorityCount);
    MDNS_LOGWARN1("::_sendMDNSMessage: additionalCount=", dnsHeader->additionalCount);
  }
  //////
